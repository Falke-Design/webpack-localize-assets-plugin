...
const get_nested_key_1 = require("./utils/get-nested-key");
...
function localizeAsset(locales, locale, assetName, placeholderLocations, fileNamePlaceholderLocations, contentHashReplacements, source, map, compilation, localizeCompiler, trackStringKeys, nestedKeys) {
    ...
    for (const { node, range, escapeDoubleQuotes } of placeholderLocations) {
        ...
        let localizedCode = (0, call_localize_compiler_1.callLocalizeCompiler)(localizeCompiler, {
            callNode: node,
            resolveKey: (key = stringKey) => {
                if (localeData[key] || !nestedKeys) {
                    return localeData[key];
                }
                return (0, get_nested_key_1.getNestedKey)(key, localeData);
            },
            emitWarning: (message) => {
...
function generateLocalizedAssets(compilation, localeNames, locales, sourceMapForLocales, trackStringKeys, localizeCompiler, nestedKeys) {
        ...
        await Promise.all(assetsWithInfo.map(async (asset) => {
            ...
            if (isJsFile.test(asset.name)) {
                ...
                await Promise.all(localeNames.map(async (locale) => {
                    ...
                    const localizedSource = localizeAsset(locales, locale, newAssetName, placeholderLocations, fileNamePlaceholderLocations, contentHashReplacements, sourceString, sourceMapForLocales.includes(locale) && map, compilation, localizeCompiler, trackStringKeys, nestedKeys);
