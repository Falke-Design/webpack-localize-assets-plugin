...
class LocalizeAssetsPlugin {
    ...
    apply(compiler) {
        ...
        compiler.hooks.thisCompilation.tap(name, (compilation, { normalModuleFactory }) => {
            ...
            if (this.singleLocale) {
                ...
                (0, multi_locale_1.generateLocalizedAssets)(compilation, this.localeNames, this.locales, this.options.sourceMapForLocales || this.localeNames, this.trackStringKeys, this.localizeCompiler, this.nestedKeys);
    ...
    interceptTranslationFunctionCalls(normalModuleFactory) {
        const { locales, singleLocale, functionNames, nestedKeys, } = this;
        const validator = (0, localized_string_key_validator_1.localizedStringKeyValidator)(locales, this.options.throwOnMissing, this.nestedKeys);
     ...
     */
    getMarkedFunctionPlaceholder(callNode, key, module, nestedKeys) {
        // Track used keys for hash
        if (!module.buildInfo.localized) {
            module.buildInfo.localized = {};
        }
        if (!module.buildInfo.localized[key]) {
            module.buildInfo.localized[key] = this.localeNames.map((locale) => {
